...
|---|---|---|---|
| P0.1 | **Webhook MP no dispara.** MP no manda POST a `/api/compras/webhook`. Defensa en profundidad cae a "solo cron" — si el cron se congela durante la campaña, procesamiento de pagos cae. | `BASE_URL=http://<IP_DROPLET>` en `.env` de prod → la `notification_url` generada era HTTP+IP, MP la rechaza. | ✅ **Fixeado 21/4 00:55**: `BASE_URL=https://sindicatoargentinodeboleros.com.ar`. Pendiente re-test y verificar que la URL configurada en panel MP también sea HTTPS+dominio (esa sobreescribe la que mandamos en payload). |
